...
<div class="br-mainpanel">
    ...
    <div class="br-pagebody">
        <div class="br-section-wrapper">
            <form method="POST" action="{{route('admin.updatetestimonial')}}" enctype="multipart/form-data" class="table-wrapper sequenceform">
                @csrf
                ...
                <div class="row mg-b-25">
                    <div class="col-lg-7">            
                        <div class="form-group">            
                            ...
                            <input type="text" class="form-control" value="{{ old('title',$data->name) }}" name="title"
                      ...
                      <div class="col-lg-5">            
                        <div class="form-group">            
                            <label>Founder <span class="error">*</span></label>            
                            <input type="text" class="form-control" value="{{ old('founder',$data->founder) }}" name="founder"
                                placeholder="Founder Here...">            
                                @error('founder') <span class="text-danger error">{{$message}}</span>  @enderror        
                        </div>            
                    </div>
                    <div class="col-2">
                      <div class="form-group">            
                        ...
                        <select name="rating" class="form-control">
                          ...
                          <option value="5" @selected(old('rating',$data->rating)==5)>5 start</option>
                          <option value="4" @selected(old('rating',$data->rating)==4)>4 start</option>
                          <option value="3" @selected(old('rating',$data->rating)==3)>3 start</option>
                          <option value="2" @selected(old('rating',$data->rating)==2)>2 start</option>
                          <option value="1" @selected(old('rating',$data->rating)==1)>1 start</option>
                        </select>       
                    ...
                    <div class="col-lg-2">                        
                      <x-admin.image-preview>
                        <x-slot:image>{{$data->image}}</x-slot>
                        <x-slot:path>/uploads/testimonial/</x-slot>
                        <x-slot:alt>{{$data->title ?? ''}}</x-slot>
...
@endsection
@push('css')
<title>Edit Testimonial : {{project()}}</title>
@endpush
@push('js')
