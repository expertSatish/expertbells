        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-7 col-md-8">
                    ...
                    <form action="{{ route('expert.saveregisteration', ['step' => 's11']) }}" method="post"
                        class="card card-body formbox mt-3" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="CustomerInfo" id="s11">
                                    <small class="num text-secondary d-block mb-3 text-center"><span class="thm">11
                                            <i class="fal fa-long-arrow-right"></i></span> Step</small>
                                    <h3 class="h5 thm">LinkedIn profile</h3>
                                    <div class="input-group">
                                        <input type="text" class="form-control"
                                            value="{{ old('linkedin', expertinfo()->linkedin) }}" name="linkedin"
                                            placeholder="https://">
                                    </div>
                                    @error('linkedin')
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control"
                                            value="{{ old('instagram', expertinfo()->instagram) }}" name="instagram"
                                            placeholder="https://">
                                    </div>
                                    @error('instagram')
                                    <span class="error">{{ $message }}</span>
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control"
                                            value="{{ old('youtube_channel', expertinfo()->youtube_channel_link) }}"
                                            name="youtube_channel" placeholder="https://">
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            ...
                                            <div class="input-group align-items-center">
                                                <label for="video"
                                                    class="small me-3 VideoL d-flex align-items-center">Add
                                                    file here <i class="fal fa-arrow-to-bottom h5 m-0 ms-3"></i></label>
                                                <input type="file" class="form-control inputTextBox" name="video"
                                                    id="video" placeholder="Type your answer here...">
                                            </div>
                                        </div>
                                        @error('video')
                                        <span class="error">{{ $message }}</span>
                                        @enderror
                                        <div class="col-sm-6">
                                        <h3 class="h5 thm mt-4">Upload Profile Pic</h3>
                                            <div class="input-group align-items-center">
                                                <label for="profile"
                                                    class="small me-3 VideoL d-flex align-items-center">Add
                                                    file here <i class="fal fa-arrow-to-bottom h5 m-0 ms-3"></i></label>
                                                <input type="file" class="form-control inputTextBox" name="profile"
                                                    id="profile" placeholder="Type your answer here...">
                                                <button class="btn ms-2 formbtn Next checkstep11 sws-top"
                                                    onclick="$('.checkstep11').hide(); $('.p-checkstep11').show();"
                                                    data-title="Next"><i class="fal fa-arrow-right"></i></button>
                                                <button type="button"
                                                    class="btn ms-2 formbtn Next p-checkstep11 sws-top disabled"
                                                    style="display:none;" data-title="Next"> <i
                                                        class="fad fa-spinner-third fa-spin"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-center mt-3">
                                        <a href="{{ route('expert.register', ['type' => 's10']) }}"
                                            class="btn btn-sm formbtn BackToStep sws-top sws-bounce" data-title=""><i
                                                class="fal fa-arrow-left me-2"></i> Back</a>
                    ...
                    <p class="text-secondary w-100 m-0 mt-5 text-center mb-3">Already have an account? <a class="thm"
...
$('input[name=linkedin]').on('change', function(e) {
    if (!
        /(ftp|http|https):\/\/?(?:www\.)?linkedin.com(\w+:{0,1}\w*@)?(\S+)(:([0-9])+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/
        .test(e.target.value)) {
        $('input[name=linkedin]').val('');
        toastr.error('Please enter valid linkedin url');
    }
});
$('input[name=instagram]').on('change', function(e) {
    if (!
        /(ftp|http|https):\/\/?(?:www\.)?instagram.com(\w+:{0,1}\w*@)?(\S+)(:([0-9])+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/
        .test(e.target.value)) {
        $('input[name=instagram]').val('');
        toastr.error('Please enter valid instagram url');
    }
});
$('input[name=youtube_channel]').on('change', function(e) {
    if (!
        /(ftp|http|https):\/\/?(?:www\.)?youtube.com(\w+:{0,1}\w*@)?(\S+)(:([0-9])+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/
        .test(e.target.value)) {
        $('input[name=youtube_channel]').val('');
        toastr.error('Please enter valid youtube channel url');
    }
});
</script>
@endpush