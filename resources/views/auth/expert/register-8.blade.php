        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-7 col-md-8">
                    ...
                    <form action="{{route('expert.saveregisteration',['step'=>'s8'])}}" method="post"
                        class="card card-body formbox mt-3">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="CustomerInfo" id="s8">
                                    <small class="num text-secondary d-block mb-3 text-center"><span class="thm">8 <i
                                                class="fal fa-long-arrow-right"></i></span> Step</small>
                                    <h3 class="h5 thm">How much do you intend to charge for the session? (INR) <span
                                            class="text-danger">*</span></h3>
                                           
                                    <div class="input-group">
                                        <input type="number"  value="{{old('charge',expertinfo()->charge)}}" class="form-control" name="charge"
                                            placeholder="for 60 MInutes Charge (INR)" onchange="myFunction(this.value)"
                                            id="price">
                                    </div><br>
                                    <div class="input-group">
                                        <input type="number" class="form-control"
                                            name="charge_30_min" id="total"/>
                                         
                                        <button class="btn ms-2 formbtn Next checkstep8 sws-top"
                                            onclick="$('.checkstep8').hide(); $('.p-checkstep8').show();"
                                            data-title="Next"><i class="fal fa-arrow-right"></i></button>
                                        <button type="button"
                                            class="btn ms-2 formbtn Next p-checkstep8 sws-top disabled"
                                    ...
                                    @if(session('danger'))
                                            ...
                                            @endif
                                    <small><b>NOTE:</b> We take a commission of 30% on the total amount of the session
                                    </small>
                                    @error('charge') <br> <span class="error">{{$message}}</span> @enderror
                                   
                                    <small class="error name-error"></small>
                                    @error('name')<small class="error">{{$message}}</small>@enderror
                                    <div class="text-center mt-3">
                                        <a href="{{route('expert.register',['type'=>'s7'])}}"
...
function myFunction(val) {
  var roundedVal = Math.round(val/2);
  $('#total').val(roundedVal);
}
</script>
</main>
@endsection
