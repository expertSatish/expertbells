    ...
    <section class="grey pt-3">
        <div class="container">
            <div class="row">
                <div class="col-md-2">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}"><i class="fal fa-home-alt mt-2"></i></a></li>
                        <li class="breadcrumb-item"><a aria-current="page">Dashboard</a></li>
                    </ol>
                </div>
                <div class="col-md-10">
                    <h3 class="ps-5 blue-text">My Packages</h3>
                </div>
            </div>
            <div class="row MainBoxAc">
                ...
                <div class="col-md-9">
                    <div class="table-responsive table-bordered" style="border-radius: 10px;">
                        <table class="table bg-white text-center">
                            <thead>
                                <tr class="blue-bg text-white">
                                    <th scope="col">#</th>
                            ...
                            <tbody class="blue-text fw-semibold">
                                @foreach($package as $key => $res)
                                <tr>
                                    <th scope="row">{{ $key + 1 }}</th>
                                    <td>{{$res->package_name}}</td>
                                    <td style="color:rgba(75, 174, 79, 1)">{{$res->prince_of_session}}</td>
                                    <td>{{$res->time}} mins</td>
                                    <td><a style="cursor: pointer;" onclick="openPopup1()"><i class="fa-regular fa-pen-to-square editform" data-id="{{$res->id}}" data-number_session="{{$res->number_session}}" data-prince_of_session="{{$res->prince_of_session}}" data-name="{{$res->package_name}}" data-time="{{$res->time}}" data-user_id="{{expertinfo()->user_id}}" data-description="{{$res->description}}" style="color: blue;"></i></a>
                                        <a href="{{url('/expert/package-delete/'.$res->id)}}" style="margin-left: 5px; cursor: pointer;">
                    ...
                    <div id="popup1" class="popup-overlay" style="border-radius:15px 15px 0 0">
                        <div class="popup-content p-3" style="border-radius:15px">
                            <!-- Content of the popup goes here -->
                            <div class="row">
                                <div class="col-2">
                                    <a id="closeBtn3" class="btn mt-2"><i class="fa-sharp fa-solid fa-arrow-left blue-text"></i></a>
                                </div>
                                <div class="col-8 text-center my-3 ">
                                    <h5>Package Edit</h5>
                                </div>
                            </div>
                            <form method="post" action="{{route('expert.packageUpdate')}}">
                                @csrf
                                <input type="hidden" name="id" id="id">
                                <input type="hidden" name="user_id" id="user_id" value="{{expertinfo()->user_id}}">
                                <div style="font-size:smaller;" class="pt-3 pb-2">Package Name</div>
                                <input type="text" class="form-control" id="package_name" name="package_name" style="background-color: rgba(241, 247, 255, 1);" maxlength="30" required>
                                <div style="float: right;font-size:x-small">Max. 30 characters</div>
                                <br>
                                <div class="row pb-3">
                                    <div class="col-md-6">
                                        <div class="py-2" style="font-size:smaller;">Number of Sessions included</div>
                                        <div>
                                            <input type="number" id="number_session" name="number_session" class="form-control" oninput="checkInputValue1(this)" style="background-color: rgba(241, 247, 255, 1);" required>
                                        </div>
                                        <div style="font-size:x-small;color:red" id="demo2"></div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="py-2" style="font-size:smaller;">Time of each Session</div>
                                        <div class="dropdown"><select class="form-control dropdown-toggle" name="time" id="time" style="background-color: rgba(241, 247, 255, 1);">
                                                <option value="30"> 30 minutes</option>
                                                <option value="60"> 60 minutes</option>
                                            </select></div>
                                    </div>
                                    <div class="py-2">
                                        <div class="pt-3 pb-2" style="font-size:smaller;">Price of the Package (INR)
                                        </div>
                                        <div class="dropdown col-6 d-flex"><input type="number" id="prince_of_session" name="prince_of_session" class="form-control" style="background-color: rgba(241, 247, 255, 1);" required></div>
                                    </div>
                                    <div style="font-size:smaller;" class="py-2">Description of the Package</div>
                                    <div>
                                        <textarea class="form-control" name="description" id="description" style="background-color: rgba(241, 247, 255, 1);"></textarea>
                                    </div>
                                    <div class="py-2 row text-center">
                                        <div class="col-6">
                                            <a id="closeBtn4" class="btn mt-2 border-primary rounded text-primary" style="width: 100%;">Cancel</a>
                                        </div>
                                        <div class="col-6">
                                            <button type="submit" class="btn mt-2 bg-primary text-white rounded" style="width: 100%;">Save</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
                    ...
                    <div id="popup" class="popup-overlay" style="border-radius:15px 15px 0 0">
                        <div class="popup-content p-3" style="border-radius:15px">
                            ...
                            <div class="row">
                                <div class="col-2">
                                    <a id="closeBtn" class="btn mt-2"><i class="fa-sharp fa-solid fa-arrow-left blue-text"></i></a>
                                </div>
                                <div class="col-8 text-center my-3 ">
                                    <h5>Package Add</h5>
                                </div>
                            </div>
                            <form method="post" action="{{route('expert.packageSave')}}">
                                @csrf
                                <input type="hidden" name="user_id" value="{{expertinfo()->user_id}}">
                                <div style="font-size:smaller;" class="pt-3 pb-2">Package Name</div>
                                <input type="text" class="form-control" name="package_name" style="background-color: rgba(241, 247, 255, 1);" maxlength="30" required>
                                <div style="float: right;font-size:x-small">Max. 30 characters</div>
                                <br>
                                <div class="row pb-3">
                                    <div class="col-md-6">
                                        <div class="py-2" style="font-size:smaller;">Number of Sessions included</div>
                                        <div>
                                            <input type="number" name="number_session" class="form-control" oninput="checkInputValue(this)" style="background-color: rgba(241, 247, 255, 1);" required>
                                        </div>
                                        <div style="font-size:x-small;color:red" id="demo1"></div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="py-2" style="font-size:smaller;">Time of each Session</div>
                                        <div class="dropdown"><select class="form-control dropdown-toggle" name="time" style="background-color: rgba(241, 247, 255, 1);">
                                                <option value="30"> 30 minutes</option>
                                                <option value="60"> 60 minutes</option>
                                            </select></div>
                                    </div>
                                    <div class="py-2">
                                        ...
                                        <div class="dropdown col-6 d-flex"><input type="number" name="prince_of_session" class="form-control" style="background-color: rgba(241, 247, 255, 1);" required>
                                        </div>
                                    </div>
                                    <div style="font-size:smaller;" class="py-2">Description of the Package</div>
                                    <div>
                                        <textarea class="form-control" name="description" style="background-color: rgba(241, 247, 255, 1);"></textarea>
                                    </div>
                                    <div class="py-2 row text-center">
                                        <div class="col-6">
                                            <a id="closeBtn2" class="btn mt-2 border-primary rounded text-primary" style="width: 100%;">Cancel</a>
                                        </div>
                                        <div class="col-6">
                                            <button type="submit" class="btn mt-2 bg-primary text-white rounded" style="width: 100%;">Save</button>
                                        </div>
                    ...
                    <a class="btn text-white blue-bg my-3" onclick="openPopup()">Add Package</a>
                </div>
            </div>
        </div>
    </section>
...
<style type="text/css">
    ...
    .MailBoxR .MBmails ul li .name {
        width: 120px
    }

    .MailBoxR .MBmails ul li .mail {
        width: calc(100% - 230px)
    }

    .MailBoxR .MBmails ul li .date {
        width: 80
    }

    .MailBoxR .MBmails ul li .attachment {
        width: 20px
    }
    ...
    .VideoBox .card {
        padding: 0;
        overflow: hidden;
        box-shadow: 0 0 16px rgb(var(--blackrgb)/.1) !important;
    ...
    .VideoBox .card .card-header .playVideo {
        position: absolute;
        height: 100%;
        width: 100%;
        left: 0;
    ...
    .VideoBox .card .card-body {
        text-align: center
    }

    .VideoBox .card .card-body h3 {
        font-size: 18px !important;
        margin: 0 0 5px;
        color: var(--thm3) !important;
        font-weight: 500;
        display: -webkit-box;
        overflow: hidden;
        -webkit-box-orient: vertical;
    ...
    .VideoBox .card:hover {
        ...
        box-shadow: 0 0 16px rgb(var(--blackrgb)/.2) !important
    }

    .VideoBox .card:hover .card-body {
        background: rgb(var(--thmrgb)/.06)
    }
...
<script>
    ...
    function getgraf() {
        $.ajax({
            ...
            method: "Post",
            dataType: 'Json',
            url: @json(route('expert.dashboardgraf')),
    ...
    function drawChart(dynamicresponse) {
        var data = google.visualization.arrayToDataTable(dynamicresponse);
        var options = {
            chart: {
                // title: 'Company Performance',
                subtitle: 'Year ' + $('#grafyear').val(),
            }
        };
...
<script>
    ...
    $(document).on('click', '.editform', function() {
        ...
        var number_session = $(this).data('number_session');
        var time = $(this).data('time');
        var prince_of_session = $(this).data('prince_of_session');
        var number_session = $(this).data('number_session');
        var description = $(this).data('description');
        $('#package_name').val(package_name);
        $('#number_session').val(number_session);
        $('#description').val(description);
        $('#time').val(time);
        $('#id').val(id);
        $('#user_id').val(user_id);
        $('#number_session').val(number_session);
        $('#prince_of_session').val(prince_of_session);
        $('#popup1').modal('show');
    });
    ...
    closeBtn3.addEventListener('click', closePopup1);
    var closeBtn4 = document.getElementById('closeBtn4');
    closeBtn4.addEventListener('click', closePopup1);

    function checkInputValue(input) {
        var a = document.getElementById("demo1");
        if (input.value > 20) {
            a.innerHTML = "Cannot be greater than 20";
        } else {
            a.innerHTML = "";
        }
    }
